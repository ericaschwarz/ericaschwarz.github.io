...
export function generateMazeWithBuildings(rows, cols, sitePages, buildingWidth = 3, buildingHeight = 3) {
  ...
  for (const page of sitePages) {
    ...
    while (!placed && tries < 200) {
      const bx = Math.floor(Math.random() * (cols - buildingWidth - 2));
      const by = Math.floor(Math.random() * (rows - buildingHeight - 2));

      if (canPlaceBuilding(maze, bx, by, buildingWidth, buildingHeight, winObjects)) {
...
export function canPlaceBuilding(maze, x, y, width, height, winObjects = []) {
...
export function placeBuilding(maze, x, y) {
